...
void tree_sector::name(const char *f, ...) {
    va_list args;
    va_start(args, f);
...
int32_t tree_sector::leaf_insert_nonfull(default_node_type *node, uint32_t &key, uint32_t &value, unsigned index) {
    assert(node->type == node_type::Leaf);
    assert(node->number_keys < default_node_type::InnerSize);
    assert(index < default_node_type::InnerSize);
...
int32_t tree_sector::leaf_node_insert(default_node_type *node, node_ptr_t node_ptr,
    ...
    if (node->number_keys >= default_node_type::LeafSize) {
        ...
        auto threshold = (default_node_type::LeafSize + 1) / 2;
...
int32_t tree_sector::inner_insert_nonfull(depth_type current_depth, default_node_type *node, uint32_t key, uint32_t value) {
    ...
    if (left != sector()) {
        ...
        auto err = db().unsafe_all([&](uint8_t *buffer, size_t size) {
            return dhara_->read(sector(), buffer, size);
        });
        if (err < 0) {
            return err;
        }
...
int32_t tree_sector::inner_node_insert(depth_type current_depth, default_node_type *node, node_ptr_t node_ptr, uint32_t key, uint32_t value, insertion_t &insertion) {
    ...
    if (node->number_keys == default_node_type::InnerSize) {
        auto treshold = (default_node_type::InnerSize + 1) / 2;
...
int32_t tree_sector::create() {
    ...
    db().emplace<default_node_type>(node_type::Leaf);
    dirty(true);
...
int32_t tree_sector::add(uint32_t key, uint32_t value) {
...
int32_t tree_sector::find(uint32_t key, uint32_t &value) {
...
int32_t tree_sector::allocate_node(default_node_type *node, node_ptr_t &ptr) {
    // logged_task lt{ "allocate" };

    db().seek_end();

    if (db().room_for<default_node_type>()) {
...
int32_t tree_sector::follow_node_ptr(node_ptr_t &ptr, persisted_node_t &followed) {
    // logged_task lt{ "follow-node" };
    ...
    if (ptr.sector != sector_) {
        ...
        err = db().unsafe_all([&](uint8_t *buffer, size_t size) {
            return dhara_->read(ptr.sector, buffer, size);
        });
        if (err < 0) {
            return err;
        }
...
int32_t tree_sector::back_to_root() {
    if (sector_ != root_) {
        ...
        assert(!dirty());
        auto err = db().unsafe_all([&](uint8_t *buffer, size_t size) {
            return dhara_->read(sector_, buffer, size);
        });
        if (err < 0) {
            return err;
        }
...
int32_t tree_sector::log(default_node_type *node) {
    logged_task it{ name() };
